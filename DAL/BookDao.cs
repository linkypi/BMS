        ...
        public Result Search(UserInfo user, SearchModel condition)
        {
            ...
            if (condition.CurrentPage == 0)
            {
                sql = "select  top " + condition.PageSize + @" 
                   BookName,SMPosition,BAuthor,BTypeID,BTypeName,Publisher,BPrice,PubDate,BID,SerialNum 
                   from Book left join TypeInfo on Book.BTypeID=TypeInfo.TypeID  ";
                sqlCount = "select count(1) from Book ";
            }
            else
            {

                sql = @"select  top " + condition.PageSize + @"  BookName,SMPosition,BAuthor,BTypeID,BTypeName,
                           Publisher,BPrice,PubDate,BID,SerialNum  from Book   left join TypeInfo on Book.BTypeID=TypeInfo.TypeID 
                           where  BID not in (select distinct top " +
                ...
                {
                    sql += " left join BookKey on Book.BID = BookKey.BID  left join TypeInfo on Book.BTypeID=TypeInfo.TypeID ";
                    sqlCount += " left join BookKey on Book.BID = BookKey.BID  left join TypeInfo on Book.BTypeID=TypeInfo.TypeID ";
                }
                foreach (var item in condition.Conditions)
                {
                    ...
                    if (item.Key == "BTypeID")
                    {
                        sql += " or " + item.Key + " = " + item.Value.ToString();
                        sqlCount += " or " + item.Key + " = " + item.Value.ToString();
                        continue;
                    }
                    sqlCount += " or " + item.Key + " like '%" + item.Value.ToString() + "%'";
                    sql += " or " + item.Key + " like '%" + item.Value.ToString() + "%'";
                }
            ...
            int total = Convert.ToInt32(DbHelper.AccessHelper.ExecuteScalarByString(sqlCount, new Dictionary<string, object>()));
            condition.AllPages = total;

            List<Book> models =  DbHelper.AccessHelper.GetDataListByString<Book>(sql,new Dictionary<string,object>(),oudic);
        ...
        public Result Add(UserInfo user,Book model)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    int count = Convert.ToInt32(DbHelper.AccessHelper.ExecuteScalarByString(sql, indic));
                    if (count >= 0)
                    {
                        ts.Complete();
        ...
        public Result Update(UserInfo user, Book model)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    indic.Add("@BookName", model.BookName);
                    ...
                    count = (int)DbHelper.AccessHelper.ExecuteScalarByString(sql, indic);
                    if (count == 1)
                    {
                        ts.Complete();
                        return new Result() { ReturnValue = true, Message = "添加成功！" };
