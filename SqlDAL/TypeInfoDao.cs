    ...
    public class TypeInfoDao : ITypeInfoDao
    {
        ...
        public Result GetList(UserInfo user)
        {
            try
            {
                ...
                List<TypeInfo> models = DbHelper.SqlHelper.GetDataListByString<Model.TypeInfo>(sql, new Dictionary<string, object>(), outdic);
        ...
        public Result SearchType(UserInfo user, SearchModel condition)
        {
            try
            {
                ...
                outdic.Add("TypeID", "TypeID");
                outdic.Add("BTypeName", "BTypeName");
                List<TypeInfo> models = DbHelper.SqlHelper.GetDataListByString<Model.TypeInfo>( sql2, new Dictionary<string, object>(), outdic);
        ...
        public Result Add(UserInfo user,string typeName)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    string sql = "select count(1) from TypeInfo where BTypeName = '" + typeName + "'";
                   ...
                   int count = (int)DbHelper.SqlHelper.ExecuteScalarByString(sql, new Dictionary<string, object>());
                    ...
                    if (count > 0)
                    {
                        return new Result() { ReturnValue = false, Message = "图书类别：" + typeName + "已存在，添加失败！" };
                    }
                    #endregion
        ...
        public Result Update(UserInfo user, TypeInfo model)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    indic.Add("@tname", model.BTypeName);
                    count = SqlHelper.ExecuteNonQuery(sql, indic);
                    if (count == 1)
                    {
                        ts.Complete();
        ...
        public Result Delete(UserInfo user, List<int> list)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    string sql = "select count(1) from Book where btypeid in ";
                    string tids = "( ";
                    int index = 1;
                    ...
                    int count = (int)DbHelper.SqlHelper.ExecuteScalarByString(sql+tids,new Dictionary<string,object>());
                    if (count > 0)
                    {
                        return new Result() { ReturnValue =false,Message="部分类别使用中无法删除，删除失败！"};
                    }
                    ...
                    int ret = (int)SqlHelper.ExecuteScalarByString(sql,new Dictionary<string,object>());
                    if (ret == -1)
                    {
                        return new Result() { ReturnValue = false, Message = "该类别已删除！" };
                    }
